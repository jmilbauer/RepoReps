...
def evalRange(X, Y, embeddings, low, high):
    counts = Counter(Y)
    topN = sorted(counts.items(), key=lambda x: x[1], reverse=True)
    if low >= len(topN):
        return -1, -1, -1
    else:
        topN = topN[low:high]
...
for i in range(0, len(Y), 100):
    f1, prec, rec = evalRange(X,Y,embeddings,i,i+100)
    print(f"{i}-{i+100}: p:{prec:.4f} r:{rec:.4f} f1:{f1:.4f}")
f1, prec, rec = evalRange(X, Y, embeddings, 0, len(Y))
print(f"{i}-{i+100}: p:{prec:.4f} r:{rec:.4f} f1:{f1:.4f}")
...
print("###")
for i in range(0, len(Y), 100):
    f1, prec, rec = evalRange(X,Y,random_embeddings,i,i+100)
    print(f"{i}-{i+100}: p:{prec:.4f} r:{rec:.4f} f1:{f1:.4f}")
f1, prec, rec = evalRange(X,Y,random_embeddings,0,len(Y))
print(f"{i}-{i+100}: p:{prec:.4f} r:{rec:.4f} f1:{f1:.4f}")